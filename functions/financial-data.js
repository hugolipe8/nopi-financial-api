...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Despesas Variáveis detalhadas (rows 28-32)
    const despesasVariaveis = [];
    const linhasDV = [
      { row: 28, nome: "Negócios" },
      { row: 29, nome: "Royalties" },
      { row: 30, nome: "Bónus" },
      { row: 31, nome: "Comissões" },
      { row: 32, nome: "Investimento" },
    ];
    for (const { row, nome } of linhasDV) {
      const r = dr[row] || [];
      const entry = { nome };
      MONTHS.forEach((m, idx) => { entry[m] = toNum(r[idx + 2]); });
      entry.media = toNum(r[15]);
      entry.total = toNum(r[14]);
      despesasVariaveis.push(entry);
    }
    ...
    return json(200, {
      ...
      dr:  { resultadoYTD, despesasFixas, despesasVariaveis, resumoDR, previsao, totalPrevisao },
    });
  } catch (err) {
